...
1. **Keep delivering, and watch the keep rate.** Deliveries 001 and 002 are live; the design pass is done; the calendar event exists. **Delivery 003 lands Monday 14 Sep.** The open question the next cycle answers is whether the ~70% keep rate is real or an artifact of Delivery 001's poor affordances — that determines whether the delivery should shrink to three twice-weekly. *(Corrected 7 Sep 2026: this sentence previously ended "Nothing else should be changed until it does," which is the evidence-gating pattern `AGENTS.md` section 3 forbids. Hold the two **caps** fixed so a later change stays attributable; everything else — the page, the copy, the selection, the tooling — changes freely.)* Record what use reveals in `docs/evals/pilot-log.md`.
